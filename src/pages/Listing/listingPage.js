...
class Listing extends Component {
    ...
    async componentDidMount() {
        ...
        let Data = await get(`http://localhost:8080/college?Department=${this.props.location.state.data}`)
        this.setState({ isLoding: false })
        this.setState({ CollegeData: Data })
        let course = await get(`http://localhost:8080/course?department=${this.props.location.state.data}`)
    ...
    async handleChange(event) {
        ...
        if (event.target.value === "") {
            ...
            let cities = await get(`http://localhost:8080/address?state=${event.target.value}`)
            this.setState({ Cities: cities })
        }
    }
    ...
    async  handleSubmit(e) {
        e.preventDefault();
        const { selectedCity, selectedCourse, selectedState, Department } = this.state
    ...
    render() {
        ...
        return (
            <div>
                ...
                <p style={{ textAlign: 'left', fontSize: 16, marginLeft: 25, marginRight: 30 }}>
                ...
                <div className={classes.root}>
                    <Card className={classes.root1} variant="none" style={{ width: 300, marginLeft: 25, marginRight: 30 }}>
                        <CardContent><Typography variant="h6" component="h9">
                            Filter College
        </Typography>
                            ...
                            <Button style={{ marginLeft: '24%', marginTop: 10 }} onClick={this.handleSubmit} variant="contained" color="primary">
                                Apply Filter
      </Button>
                    ...
                    <GridList cellHeight={160} className={classes.gridList} cols={2}>
                        ...
                        {this.state.CollegeData.map((college) => (
                            ...
                            <Card className={classes.root1} style={{ width: 450, height: 280, marginLeft: 30, marginBottom: 30 }}>
                                <CardHeader
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: 'inline-block', marginLeft: 70 }}>
                                            ...
                                            <Typography className={classes.pos} color="textSecondary">
                                                ...
                                                <a href={college.Website} rel="noopener noreferrer" target="_blank">{college.Website}</a>
                                            </Typography>
                                        </div>
